...
to write subsequent code in the structured [`hx0` format](specs/hx0.md).
...
1. **Beginning:** Manually enter the hex found in `seed.hx0` into a binary file
   ...
   contents match).
1. **Naming Convention:** While it is conceptually `seed.efi`, UEFI firmware
   strictly requires the default bootloader to be named `BOOTRISCV64.EFI`. Save
   the file to `/EFI/BOOT/BOOTRISCV64.EFI` on the FAT drive.
1. **Setup:** Place your target [`hx0` format](specs/hx0.md) file in the root of
   the FAT-formatted virtual drive and name it `src.hx0` (this filename is
   hardcoded).
